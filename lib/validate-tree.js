...
const validateNoOrphanNodes = tree => {
  ...
  const optionReferences = _.uniq(
    _.concat(
      [tree.startNodeId],
      _.flatten(
        _.compact(nodes.map(node => node.options && node.options.map(option => option.nextNodeId)))
      )
    )
  );
  ...
  if (orphans.length > 0) {
    throw new Error(e(`Unreferenced nodes found (${orphans.join(', ')})`));
  }
};
...
const validateOption = (tree, node, option) => {
  ...
  if (option.nextNodeId !== '') {
    if (!_.find(tree.nodes, node => node.id === option.nextNodeId)) {
      throw new Error(e(`${node.title}: option points to a missing node (${option.nextNodeId}).`));
    }
  }
};
